...
class FemnistCNN_small(nn.Module):
    """ CNN Network architecture. """

    def __init__(self, output_dim):
        super(FemnistCNN_small, self).__init__()
        self.net = nn.Sequential(
            nn.Conv2d(1, 16, 5, padding=2, stride=1),
            nn.ReLU(),
            nn.MaxPool2d(2, stride=2),
            nn.Conv2d(16, 64, 5, padding=2, stride=1),
            nn.ReLU(),
            nn.MaxPool2d(2, stride=2),
            nn.Flatten(),
            nn.Linear(7 * 7 * 64, 320),
            nn.ReLU(),
